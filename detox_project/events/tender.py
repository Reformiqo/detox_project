...
def _create_project_from_tender(doc):
    ...
    if existing:
        frappe.msgprint(
            _("Project <b>{0}</b> already exists for this Tender.").format(existing),
            indicator="blue"
        )
        return
    ...
    frappe.msgprint(
        ...
        indicator="green", title=_("Project Created")
    )
...
def _cancel_project_cascade(doc, reason=""):
    ...
    if frappe.db.exists("DocType", "Financial Model"):
        fms = frappe.get_all("Financial Model", filters={"project": project_name, "docstatus": 1}, pluck="name")
        for fm_name in fms:
            try:
    ...
    if frappe.db.exists("DocType", "Project Budget Plan"):
        pbps = frappe.get_all("Project Budget Plan", filters={"project": project_name, "docstatus": 1}, pluck="name")
    ...
    frappe.msgprint(
        ...
        indicator="orange", title=_("Cascade Cancel Complete")
    )
...
def cascade_cancel_project(project_name, reason="Manual Cancellation"):
    ...
    if tender_name and frappe.db.exists("Tender Management", tender_name):
        ...
        if frappe.db.exists("DocType", "Financial Model"):
            for fm_name in frappe.get_all("Financial Model", filters={"project": project_name, "docstatus": 1}, pluck="name"):
                try:
                    fm_doc = frappe.get_doc("Financial Model", fm_name)
                    fm_doc.flags.from_cascade = True
                    fm_doc.cancel()
        ...
        if frappe.db.exists("DocType", "Project Budget Plan"):
            for pbp_name in frappe.get_all("Project Budget Plan", filters={"project": project_name, "docstatus": 1}, pluck="name"):
                try:
                    pbp_doc = frappe.get_doc("Project Budget Plan", pbp_name)
                    pbp_doc.flags.from_cascade = True
                    pbp_doc.cancel()
        ...
        for wbs_name in frappe.get_all("WBS Element", filters={"project": project_name, "status": ["!=", "Cancelled"]}, pluck="name"):
            frappe.db.set_value("WBS Element", wbs_name, "status", "Cancelled")

        for sub_name in frappe.get_all("Sub WBS Element", filters={"project": project_name, "status": ["!=", "Cancelled"]}, pluck="name"):
