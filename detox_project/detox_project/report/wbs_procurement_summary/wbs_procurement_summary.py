...
def get_columns():
    return [
        {"fieldname": "wbs_element", "label": _("WBS Element"), "fieldtype": "Link", "options": "WBS Element", "width": 150},
...
def get_data(filters):
    ...
    for wbs in wbs_elements:
        for mr in frappe.get_all(
            ...
            data.append({
                "wbs_element": wbs.name, "wbs_name": wbs.wbs_name, "project": wbs.project,
                "doc_type": "Material Request", "doc_name": mr.name,
                "supplier": "", "date": mr.date, "grand_total": 0, "status": mr.status,
            })
        ...
        for po in frappe.get_all(
            ...
            data.append({
                "wbs_element": wbs.name, "wbs_name": wbs.wbs_name, "project": wbs.project,
                "doc_type": "Purchase Order", "doc_name": po.name,
                "supplier": po.supplier, "date": po.date, "grand_total": po.grand_total, "status": po.status,
            })
        ...
        for pi in frappe.get_all(
            ...
            data.append({
                "wbs_element": wbs.name, "wbs_name": wbs.wbs_name, "project": wbs.project,
                "doc_type": "Purchase Invoice", "doc_name": pi.name,
                "supplier": pi.supplier, "date": pi.date, "grand_total": pi.grand_total, "status": pi.status,
            })
