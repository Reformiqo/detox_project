// Same logic as Material Request — adapted for Purchase Invoice
...
frappe.ui.form.on("Purchase Invoice", {
	...
	refresh(frm) {
		...
		calculate_wbs_allocated_amounts_pi(frm);
	},
});

frappe.ui.form.on("Purchase Invoice Item", {
	custom_wbs_element(frm) { calculate_wbs_allocated_amounts_pi(frm); },
	custom_sub_wbs_element(frm) { calculate_wbs_allocated_amounts_pi(frm); },
	amount(frm) { handle_single_wbs_auto_assign_pi(frm); calculate_wbs_allocated_amounts_pi(frm); },
	rate(frm) { handle_single_wbs_auto_assign_pi(frm); calculate_wbs_allocated_amounts_pi(frm); },
	qty(frm) { handle_single_wbs_auto_assign_pi(frm); calculate_wbs_allocated_amounts_pi(frm); },
	items_remove(frm) { calculate_wbs_allocated_amounts_pi(frm); },
});

frappe.ui.form.on("WBS Allocation", {
	wbs_element(frm) { handle_single_wbs_auto_assign_pi(frm); calculate_wbs_allocated_amounts_pi(frm); },
	sub_wbs_element(frm) { handle_single_wbs_auto_assign_pi(frm); calculate_wbs_allocated_amounts_pi(frm); },
	custom_wbs_allocations_remove(frm) { calculate_wbs_allocated_amounts_pi(frm); },
});

function handle_single_wbs_auto_assign_pi(frm) {
